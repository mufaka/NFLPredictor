...
The full per-`(combination, slice)` metric matrix. One JSON object with a top-level `"combinations"` key; each combination id maps to a slice → metric-cell dict. For an **S1** combination the slices are `"val"` and `"test"`; for an **S3** combination they are `"fold_0".."fold_<n-1>"` plus `"pooled"`. Each cell carries `n_games` plus the five metric families:

* `mae` — the **headline metric**, per-side averaged: `mean(|pred_home − true_home| + |pred_away − true_away|) / 2`. Numerically identical to Phase 4's `training_summaries.<combo>.val_mae` for S1 val cells (the cross-check is enforced by `tests/test_evaluate_cross_phase.py`).
...
* `n_games` cell values that don't match the expected slice size from `splits_2024.json`. Indicates a join bug between predictions and labels.
...
Per-team MAE should cluster around the league-wide value reported in `metrics_headline.json` for the same combination/slice. A spread of ±2–4 points across teams within a slice is normal small-sample noise (each team contributes at most one game per S1 val slice; the spread is high because `n_games` is tiny per cell). On `pooled` slices for S3 combinations, the per-team `n_games` grows and the spread tightens.
...
* `n_games == 0` for a team in a role you'd expect to be populated. Means the team had no games in that slice — usually fine for S1 val (small slice) but suspicious for `pooled`.
...
One row per `(combination_id, slice, week)` cell, where `week` is the integer 1–18 read from `features_flat_2024.parquet`'s `week` column. Each game contributes one row to its week's cell. The parquet is universe-complete across all 18 weeks even when a slice contains only a subset.

#### What good looks like

Per-week MAE should be roughly stationary across the season. Some week-to-week noise is expected (each S1 val week contains only ~15 games), but a clear *trend* — e.g., MAE rising linearly week-over-week — is not. On `pooled` slices the noise damps and any real seasonal effect becomes more visible.

#### Red flags

* MAE monotonically increasing across the val slice's weeks (13 → 14 → 15) on the **S1** combinations. Possibly the model is overfit to the early-season training data and degrades as the season's underlying distribution shifts (injuries, eliminated teams resting starters).
...
* For monotonic per-week degradation, the **S3** strategy is the proximate test: if the per-fold MAE rises with `k` (more training data, more recent val slice), the model is generalizing in time; if not, the model is memorizing recent weeks.
...
* For misordered indoor/outdoor MAE, inspect the weather columns directly in `features_flat_2024.parquet` for indoor games — are they `null`-filled, sentinel-filled, or carrying real (incorrect) values? The fix is in Phase 2, not Phase 4.
...
One file per S1 val/test slice (`__val__scatter.png`, `__test__scatter.png`) and one per S3 pooled slice (`__pooled__scatter.png`).

**Representative example:**

![scatter — rung2_linear flat S1 val](../Data/processed/evaluation/plots/rung2_linear__flat__s1__val__scatter.png)
...
One file per S1 val/test slice and one per S3 pooled slice.

**Representative example:**

![residuals — rung2_linear flat S1 val](../Data/processed/evaluation/plots/rung2_linear__flat__s1__val__residuals.png)
...
![by_week — rung2_linear flat S1 val](../Data/processed/evaluation/plots/rung2_linear__flat__s1__val__by_week.png)

#### What good looks like

A roughly flat line with week-over-week noise (each S1 val week has ~15 games, so single-week noise is large). On `pooled` slices for S3 combinations the noise damps; a flat line on `pooled` is the gold standard. No monotonic trend.
...
* For monotonic per-week degradation, the immediate diagnostic is to plot the same line for an **S3** combination — if S3's per-fold MAE is stable as `k` increases, the model is generalizing; if S3 trends too, the issue is in the data.
...
* The flat and pos bars for the same learned rung differ by more than 2 points. One representation is dominating; investigate why (`features_pos_2024.parquet`'s per-position aggregation may be losing information that `features_flat_2024.parquet` retains, or vice versa).
...
1. **Rung-to-rung delta on the same shape.** E.g., `rung3_mlp__flat__s1.val.mae` vs. `rung2_linear__flat__s1.val.mae`. Answers: *did the extra capacity earn its keep?* A delta of < 0.2 points on this dataset (272 games) is essentially noise; you need ≥ 0.5 to call it a real improvement and ≥ 1.0 to call it large.

2. **Shape delta on the same rung.** E.g., `rung2_linear__pos__s1.val.mae` vs. `rung2_linear__flat__s1.val.mae`. Answers: *is the position-grouped feature shape more informative than the slot-indexed shape?* This is the cleanest A/B in the entire ladder because the only thing changing is the feature layout. A consistent shape preference across rungs and slices is real signal.

3. **Strategy delta on the same combination.** E.g., `rung2_linear__flat__s1.val.mae` vs. `rung2_linear__flat__s3.pooled.mae`. Answers: *does the expanding-window protocol agree with the fixed-week-boundary protocol about which combinations win?* Disagreement is a flag that the S1 val slice is small enough to be lucky in one direction.
...
* **combination_id** — the filename stem of a Phase 4 prediction parquet, e.g., `rung2_linear__flat__s1`. Encodes rung (`rung0_mean`, `rung1_team_mean`, `rung2_linear`, `rung3_mlp`), feature shape (`flat`, `pos`, or `none` for the rungs that don't take features), and splits strategy (`s1` or `s3`). Spec source: Spec-Phase4 §3.2 (TR-OUT-01) and Spec-Phase5 §3.5 (EV-COMB-01).

* **slice** — the named subset of games over which a metric is computed. For S1 combinations: `"val"` or `"test"`. For S3 combinations: `"fold_0".."fold_<n-1>"` (per-fold val metrics) or `"pooled"` (concatenated-fold metrics). Spec source: Spec-Phase5 §3.5 (EV-COMB-02 / EV-COMB-03).

* **fold** — one of the nine expanding-window `(train, val)` partitions S3 emits. Each fold is identified by its `k` value (the last week included in train; v1 uses `k ∈ {6..14}`) and its zero-based `fold_index`. Spec source: Spec-Phase3 §3.4 and Spec-Phase4 §3.5.
...
* **strategy** — S1 (fixed week boundaries) or S3 (expanding-window folds). The third axis of the combination matrix, alongside rung and feature shape. Spec source: Spec-Phase3 §3.4.
