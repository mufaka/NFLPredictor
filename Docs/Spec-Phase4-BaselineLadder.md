...
> **Revision note (multi-year).** Revised for the six-season (2020–2025) migration per `Docs/Plan-MultiYear-Migration.md`. The two split strategies are now **`season_holdout`** (single fixed train/val/test partition) and **`loso_cv`** (leave-one-season-out CV), replacing the week-based `S1`/`S3`; the `TR-STRAT-*` requirement IDs are preserved but their semantics follow Phase 3's redesigned contract. Inputs are the combined `features_*_all.parquet` / `splits_all.json`; the feature matrices carry a `season` identifier column that the encoder excludes from the model. `training_version` is `v4`; the default `strategies` is `[season_holdout]` (loso_cv is opt-in at ≈6× cost). Some prose still says "v1" where it means the baseline ladder design, which is unchanged.
...
- Two split-strategy regimes: **season_holdout** (single fold; produces val + test predictions) and **loso_cv** (leave-one-season-out CV; produces per-fold val predictions only).
...
- **Test-slice metric computation.** Phase 4 emits season_holdout test predictions to disk but does not compute test MAE; Phase 5 owns that.
- **Rung selection.** Phase 4 reports per-rung val MAE; the human (or Phase 5) decides which rung is the "chosen" model.
- **Re-training on `the holdout train ∪ val sets` before test scoring.** The season_holdout test predictions Phase 4 emits come from a model trained on the holdout train set only. A "train on all pre-test data" recipe is a Phase 5 or follow-on amendment.
...
| Feature shape | One of `flat` (Phase 2's `features_flat_all.parquet`), `pos` (Phase 2's `features_pos_all.parquet`), or `none` (no feature inputs — rungs 0 and 1 only). |
...
| Strategy | One of `season_holdout` or `loso_cv`, named identically to Phase 3's split-strategy contract. |
| Slice | One of `train`, `val`, `test`. Train is read-only; val and test receive predictions. |
| Fold | An loso_cv `(train, val)` pair, indexed `0..4` for the default (one per rotation-pool season). |
...
| Predictions artifact | A parquet at `Data/processed/predictions/<combo>.parquet` holding one prediction row per `(slice, GameId)` for season_holdout, or `(fold_index, GameId)` for loso_cv. |
| Training manifest | `Data/processed/training_manifest.json` — provenance for the training run. |
| Headline metric | Val MAE on the season_holdout.val slice. Used for ladder gating; computed once per `(rung, shape)` for season_holdout and once per fold for loso_cv. |
| Tiebreaker | When two `(rung, shape)` combinations are within a small margin on season_holdout.val MAE, the loso_cv mean val MAE across all 5 folds is the tiebreaker. Phase 4 emits the inputs; the comparison is a human or Phase 5 act. |
...
- **No checkpoints in v1.** Re-training is cheap at ≈1,622 games × 4 rungs × 2 shapes × 2 strategies. Predictions are the deliverable; weights are throwaway.
...
- **Test slice is read-only-with-care.** Phase 4 *predicts* on season_holdout.test (so the artifact exists for Phase 5) but never *measures* it. The manifest carries val MAE only.
...
Data/processed/{splits_all.json, splits_manifest.json}
    ...
    ▼
Data/processed/predictions/{<rung>__<shape>__<strategy>.parquet,
                            training_manifest.json}
```

The Phase 4 build refuses to run if any of Phase 2's tracked outputs (`features_flat_all.parquet`, `features_pos_all.parquet`, `feature_vocab.json`) or Phase 3's output (`splits_all.json`) is missing or has an on-disk SHA-256 that disagrees with its upstream manifest. This guarantees that prediction files are always pinned to a specific Phase 2 build and a specific Phase 3 split.
...
| ID | Requirement |
|----|-------------|
| TR-IN-01 | The build shall read `Data/processed/features_flat_all.parquet`, `Data/processed/features_pos_all.parquet`, `Data/processed/feature_vocab.json`, and `Data/processed/feature_manifest.json` as Phase 2 inputs. |
| TR-IN-02 | The build shall read `Data/processed/splits_all.json` and `Data/processed/splits_manifest.json` as Phase 3 inputs. |
...
| TR-IN-06 | The build shall recompute the SHA-256 of `splits_all.json` and compare it to the corresponding `output_sha256` entry in `splits_manifest.json`. On mismatch the build shall fail fast. |
| TR-IN-07 | The build shall reject (fail-fast) any `training_config.yaml` whose schema does not satisfy §3.2. |
| TR-IN-08 | The build shall verify that `splits_all.json` contains both `season_holdout` and `loso_cv` strategies. If a required strategy is absent for any combination the config requests, the build shall fail fast. |
...
| TR-CFG-06 | `strategies` is a non-empty list. Each entry shall be one of `season_holdout`, `loso_cv`. Order is preserved. Duplicate entries shall cause the build to fail fast. |
...
| TR-SHAPE-02 | The `flat` shape shall read `features_flat_all.parquet`. The `pos` shape shall read `features_pos_all.parquet`. The build shall not synthesize features across the two files. |
...
| TR-TRAIN-05 | After training a learned rung, the build shall use the best-epoch parameters to produce predictions on the appropriate val and (for season_holdout only) test slices, per §3.9. |
...
| ID | Requirement |
|----|-------------|
| TR-STRAT-01 | **season_holdout strategy.** For each combination, the build shall train on the holdout train set from `splits_all.json` and predict on both the holdout val set and the holdout test set. Both prediction sets are written into the same `<combo>__season_holdout.parquet` file, distinguished by the `slice` column (§4.2). |
| TR-STRAT-02 | **loso_cv strategy.** For each combination, the build shall train one model per fold using fold `i`'s `train` partition, and predict on fold `i`'s `val` partition. Predictions across all folds are written into a single `<combo>__loso_cv.parquet` file, distinguished by the `fold_index` column (§4.2). |
| TR-STRAT-03 | The loso_cv build shall not produce predictions for the loso_cv test slice. Phase 5 uses season_holdout's test predictions for any test-slice evaluation. |
| TR-STRAT-04 | For trivial rungs (mean, team_mean), the same training-then-prediction protocol applies: rung 0 and rung 1 compute their summary statistics from each strategy's per-fold (or single season_holdout) training partition. Trivial rungs produce val and (for season_holdout) test predictions analogous to learned rungs. |

### 3.10 Output Encoding

| ID | Requirement |
|----|-------------|
| TR-OUT-01 | The build shall write one parquet file per `(rung, shape, strategy)` combination into `Data/processed/predictions/`. The filename shall be `<rung_id>__<shape>__<strategy>.parquet`, where `<rung_id>` is one of `rung0_mean`, `rung1_team_mean`, `rung2_linear`, `rung3_mlp`; `<shape>` is one of `none`, `flat`, `pos`; `<strategy>` is one of `season_holdout`, `loso_cv`. |
| TR-OUT-02 | For season_holdout combinations, the parquet shall have columns: `slice` (string, one of `"val"` or `"test"`), `GameId` (string), `pred_home` (float64), `pred_away` (float64). Rows shall be sorted by `(slice, GameId)` with `slice` ordering `val` before `test`. |
| TR-OUT-03 | For loso_cv combinations, the parquet shall have columns: `fold_index` (int8), `GameId` (string), `pred_home` (float64), `pred_away` (float64). Rows shall be sorted by `(fold_index, GameId)` with `fold_index` ascending. |
| TR-OUT-04 | The set of `GameId` values in each season_holdout parquet's `slice = "val"` rows shall equal `splits_all.json → season_holdout.val`. The set of `GameId` values in each season_holdout parquet's `slice = "test"` rows shall equal `splits_all.json → season_holdout.test`. No duplicates within a slice. |
| TR-OUT-05 | For each loso_cv parquet, the set of `GameId` values whose `fold_index = i` shall equal `splits_all.json → loso_cv.folds[i].val`. No duplicates within a fold. |
...
| TR-MAN-02 | `training_summaries` shall be an object keyed by combination id (the parquet filename without extension). For season_holdout entries the value is `{"val_mae": <float>, "epochs_trained": <int or null>, "best_epoch": <int or null>, "stopped_early": <bool or null>}`. For loso_cv entries the value is `{"fold_count": <int>, "mean_val_mae": <float>, "per_fold": [{"fold_index": i, "val_mae": <float>, "epochs_trained": <int or null>, "best_epoch": <int or null>, "stopped_early": <bool or null>}, …]}`. For trivial rungs, `epochs_trained`, `best_epoch`, and `stopped_early` shall be `null`. |
...
| TR-LC-04 | `training_version` is is `"v4"` to record the schema change. Cross-references DD-LC-04. |
...
**season_holdout schema** (filename ends with `__season_holdout.parquet`):
...
**loso_cv schema** (filename ends with `__loso_cv.parquet`):

| Column | Type | Description |
|--------|------|-------------|
| `fold_index` | int8 | Phase 3 fold index, `0..fold_count-1` (5 folds for the default rotation pool). |
| `GameId` | string | Phase 1 / Phase 2 `GameId`. |
| `pred_home` | float64 | Predicted home score. |
| `pred_away` | float64 | Predicted away score. |
...
|------|------|-------------|
| `Data/processed/predictions/<combo>__season_holdout.parquet` × 6 | Parquet | One per `(rung, shape)` combination with `strategy = season_holdout`. Holds val + test predictions. |
| `Data/processed/predictions/<combo>__loso_cv.parquet` × 6 | Parquet | One per `(rung, shape)` combination with `strategy = loso_cv`. Holds per-fold val predictions. |
| `Data/processed/training_manifest.json` | JSON | Build provenance + per-combination val MAE summary. |

With the default `strategies: [season_holdout]` the build emits 6 prediction parquets: trivial rungs `{rung0_mean, rung1_team_mean}` × `{none}` × `{season_holdout, loso_cv}` = 4 files; learned rungs `{rung2_linear, rung3_mlp}` × `{flat, pos}` × `{season_holdout, loso_cv}` = 8 files. The exact count is a function of the config; trimming `rungs`, `shapes`, or `strategies` in `training_config.yaml` reduces the output set proportionally without a code change.
...
2. **Load features and splits into memory.** Read both Phase 2 parquets and `splits_all.json`. Verify labels are identical across `flat` and `pos` parquets (TR-NF-02).
...
5. **Per combination, train and predict.**
   - For trivial rungs: compute closed-form summary stats from the strategy's training partition(s); emit predictions for val (and test, for season_holdout).
   - For learned rungs: seed RNGs (TR-TRAIN-02); for each fold (loso_cv) or once (season_holdout), instantiate the model, run the early-stopping training loop, predict using best-epoch params.
6. **Compute val MAE.** Per combination (season_holdout) or per fold (loso_cv), record the val MAE that goes into the manifest.
...
- **Split-strategy consumption**: both `season_holdout` and `loso_cv` are consumed. season_holdout's val MAE is the headline. loso_cv's per-fold mean val MAE is the tiebreaker for close `(rung, shape)` comparisons. The test slice is touched once at project end (in Phase 5), via season_holdout's emitted test predictions.
...
- **Output artifacts**: predictions only; no checkpoints in v1. Six parquets (default config) covering every `(rung, shape, strategy)` combination, plus a single `training_manifest.json`.
...
| TR-NF-02 | At startup, the build shall verify that `features_flat_all.parquet` and `features_pos_all.parquet` carry identical `(GameId, home_score, away_score)` triples (same set, same label values). On mismatch the build shall fail fast. |
...
| TR-NF-07 | The build shall log per-combination val MAE (and for loso_cv, per-fold val MAE and mean val MAE) to stdout in a stable, scannable format so the ladder summary is visible without opening the manifest. |
...
| TR-TEST-05 | A unit test shall verify the prediction parquet schemas (§4.2): season_holdout files have columns `(slice, GameId, pred_home, pred_away)` and contain exactly the expected `GameId` sets per slice (TR-OUT-04); loso_cv files have columns `(fold_index, GameId, pred_home, pred_away)` and exactly cover the expected per-fold sets (TR-OUT-05); rows are sorted per TR-OUT-02 / TR-OUT-03. |
...
| TR-TEST-07 | A determinism test shall run `run_training_build` twice in succession against identical Phase 2, Phase 3, and config inputs and assert byte-equality of all every prediction parquet and the manifest (excluding `build_timestamp_utc`) in the same pinned PyTorch wheel on CPU. |
| TR-TEST-08 | A pinned-identity test shall run the real Phase 4 build against the actual Phase 2 and Phase 3 outputs and assert: (a) all expected parquet filenames exist; (b) each season_holdout parquet covers exactly the the holdout val set and the holdout test set GameId sets; (c) each loso_cv parquet covers exactly the expected per-fold val GameId sets across all 5 folds; (d) the manifest carries a val MAE for every combination and no test MAE for any combination. |
| TR-TEST-09 | A source-pinning test shall verify that a TR-IN-05 hash mismatch (e.g., a hand-edited `features_flat_all.parquet`) and a TR-IN-06 hash mismatch (e.g., a hand-edited `splits_all.json`) each cause the build to fail fast with a clear error naming the divergent file. |
...
- **Per-epoch loss-curve instrumentation.** *Landed via Phase 6 amendment (Spec-Phase6 DD-LC-*; this spec's §3.12 / TR-LC-*).* The training build now emits `Data/processed/training_loss_curves.parquet` alongside the prediction parquets; `training_version` is `"v4"` for the multi-year migration. The optimizer trajectory and the prediction parquets are unchanged (DD-LC-08 / TR-LC-08).
...
- **Re-training on `the holdout train ∪ val sets` before test scoring.** Currently the season_holdout test predictions Phase 4 emits come from a model trained only on the holdout train set. A follow-on amendment may add an optional re-train step that consumes `the holdout train ∪ val sets` and re-emits the test parquet rows.
...
- **Multi-GPU / distributed training.** Out of scope for v1. Single-device (CPU or one CUDA device) is sufficient at ≈1,622 games; `DataParallel` / `DistributedDataParallel` would be follow-on amendments if the dataset grows enough to justify them.
...
- `Data/processed/features_flat_all.parquet`, `Data/processed/features_pos_all.parquet`, `Data/processed/feature_vocab.json` — Phase 2 outputs; primary inputs to Phase 4.
- `Data/processed/splits_all.json` — Phase 3 output; secondary input to Phase 4.
