...
| TR-CAT-06 | `d_in` shall be derived from Phase 2's feature parquet column set plus `feature_vocab.json`, not hard-coded. A column-set change in Phase 2 propagates automatically to the model's input layer. |
...
| TR-NF-04 | The build shall complete in under 5 minutes on a modern laptop (8-core CPU, 16 GB RAM) for the v1 ladder against the 272-game 2024 universe. |
...
- **Checkpoint persistence.** If a downstream phase (Phase 6 error analysis, or external sharing) needs the trained weights, persisting `state_dict` per learned combination is a follow-on amendment. v1 deliberately omits this; re-training is cheap.
...
- **Per-rung loss functions.** v1 standardizes on MAE. Mixing MSE/Huber across rungs to compare loss-shape effects would be a Phase 6-flavored experiment, not a Phase 4 v1 requirement.
