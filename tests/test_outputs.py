...
def test_build_mapping_records_dedupes_by_pair():
    ...
    records = build_mapping_records([(s1, r2), (s2, r3)])
...
def test_build_mapping_records_appends_position_mismatch_suffix():
    ...
    records = build_mapping_records([(s, r)])
...
def test_rewrite_box_score_ids_replaces_per_slot_ids():
    df = pd.DataFrame([
        {
            "GameId": "g1", "HomeTeamCode": "kan", "AwayTeamCode": "buf",
...
def test_write_mapping_is_sorted(tmp_path):
    from nflpredictor.databuild.outputs import MappingRecord

    path = tmp_path / "mapping.csv"
    records = [
        MappingRecord("Bbbb", "2024-00002", "x"),
        MappingRecord("Aaaa", "2024-00001", "y"),
        MappingRecord("Aaaa", "2024-00002", "z"),
    ]
